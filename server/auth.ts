...
export type AuthenticatedUser = { id: string; sessionId: string; email: string; organizationId: string; countryCode: string; roles: AppRole[]; permissions: string[] };
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS resource_shares (id TEXT PRIMARY KEY, resource_type TEXT NOT NULL, resource_id TEXT NOT NULL, owner_user_id TEXT NOT NULL REFERENCES users(id), recipient_user_id TEXT REFERENCES users(id), recipient_organization_id TEXT REFERENCES organizations(id), permission_scope TEXT NOT NULL, purpose TEXT NOT NULL, expires_at TEXT, revoked_at TEXT, created_at TEXT NOT NULL);
  CREATE TABLE IF NOT EXISTS delegated_access_grants (id TEXT PRIMARY KEY, grantor_user_id TEXT NOT NULL REFERENCES users(id), delegate_organization_id TEXT NOT NULL REFERENCES organizations(id), farm_id TEXT NOT NULL REFERENCES farms(id), permission_scope TEXT NOT NULL, purpose TEXT NOT NULL, expires_at TEXT, revoked_at TEXT, created_at TEXT NOT NULL);
...
`);
try { db.exec("ALTER TABLE sessions ADD COLUMN last_seen_at TEXT"); } catch { /* Existing deployments already migrated. */ }
...
const signupRoles = new Set<AppRole>(["farmer", "buyer", "cooperative_admin"]);
export function register(email: string, password: string, organizationName: string, countryCode: string, role: AppRole) {
  const normalizedEmail = email.trim().toLowerCase();
  if (!/^\S+@\S+\.\S+$/.test(normalizedEmail) || password.length < 12 || !/^[A-Z]{2}$/.test(countryCode) || !signupRoles.has(role)) throw new Error("Use a valid email, a password of at least 12 characters, a two-letter country code, and an allowed organization role.");
  ...
  try {
    ...
    db.prepare("INSERT INTO memberships (id, user_id, organization_id, role_id, delegated_consent) VALUES (?, ?, ?, ?, 1)").run(id("mem"), userId, organizationId, role);
    db.exec("COMMIT");
...
function userForId(userId: string, sessionId: string): AuthenticatedUser | null {
  ...
  return row ? { id: row.id, sessionId, email: row.email, organizationId: row.organization_id, countryCode: row.country_code, roles: row.roles.split(",") as AppRole[], permissions: row.permissions?.split(",") || [] } : null;
}
...
export function signIn(email: string, password: string, mfaCode?: string) {
  ...
  db.prepare("INSERT INTO sessions (id, user_id, token_hash, expires_at, revoked_at, created_at, last_seen_at) VALUES (?, ?, ?, ?, NULL, ?, ?)").run(sessionId, user.id, hashToken(token), expiresAt, new Date().toISOString(), new Date().toISOString());
  audit(user.id, "identity.sign_in", "session", sessionId);
  return { token, expiresAt, user: userForId(user.id, sessionId)! };
}
...
export function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  const session = db.prepare("SELECT id, user_id FROM sessions WHERE token_hash = ? AND revoked_at IS NULL AND expires_at > ?").get(hashToken(token), new Date().toISOString()) as { id: string; user_id: string } | undefined;
  const user = session && userForId(session.user_id, session.id);
  ...
  db.prepare("UPDATE sessions SET last_seen_at = ? WHERE id = ?").run(new Date().toISOString(), session!.id); req.auth = user; next();
}
...
export function revokeSession(sessionId: string, actorUserId: string) { db.prepare("UPDATE sessions SET revoked_at = ? WHERE id = ? AND revoked_at IS NULL").run(new Date().toISOString(), sessionId); audit(actorUserId, "identity.logout", "session", sessionId); }
...
export function listAuditLogs(limit = 100) { return db.prepare("SELECT id, occurred_at, actor_user_id, action, resource_type, resource_id, metadata_json FROM audit_logs ORDER BY occurred_at DESC LIMIT ?").all(limit); }
